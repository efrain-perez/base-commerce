...
**Postgres credentials are the same simple values already used in `backend/compose.yaml`** (`mydatabase`/`myuser`/`secret`) — not a new secret to manage for a local challenge submission; a real production deployment would obviously not hardcode these.
